...
public class DrawingLevel : MonoBehaviour
{
		...
		private void GameObjectClickHandle (GameObject ob)
		{
				...
				if (drawResizerBackgroundAnimator.GetBool ("isFadingIn")) {
						...
						if (!isRunningForward) {
								GameObject.Find ("Shape").GetComponent<Collider2D>().enabled = false;
		...
		private void BackToHome ()
		{
				...
				SceneManager.LoadScene(Configuration.shapesGridSceneName);
		}
		...
		private void CreateShape ()
		{
				...
				if (menuWindowAnimator.GetBool ("isRunningForward")) {
						shape.GetComponent<Collider2D>().enabled = false;//disable shape collider
				}
