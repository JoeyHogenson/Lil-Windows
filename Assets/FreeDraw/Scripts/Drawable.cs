        ...
        void Update()
{
    ...
    {
        // 1. Shoot a ray from the camera through the mouse position
        Ray ray = Camera.main.ScreenPointToRay(Input.mousePosition);
        RaycastHit hit;

        // 2. We use Physics.Raycast (3D) instead of Physics2D.OverlapPoint
        if (Physics.Raycast(ray, out hit, 100f, Drawing_Layers))
        {
            // Check if we hit THIS specific object
            if (hit.transform == this.transform)
            {
                // hit.point is the exact 3D coordinate on the surface
                current_brush(hit.point);
            }
        }
        else
        {
            // We missed the canvas
